...
def html_to_pdf(html_path, pdf_path):
    async def main():
        ...
        await page.pdf({
            ...
            'footerTemplate': '<div style="text-align:center;width:297mm;font-size: '
                              '8px;"><span class="pageNumber"></span>/<span '
...
class ReportMD():
    def __init__(self, project_id):
        ...
        self.media_path = settings.MEDIA_ROOT + "/reportmd_temp/{}/media".format(
            self.project_name
        )
    ...
    def work(self):
        ...
        for d in data:
            ...
            for d2 in data_2:
                ...
                with open('{}/{}.md'.format(self.project_path, md_name_2), 'w',
                ...
                for d3 in data_3:
                    ...
                    with open('{}/{}.md'.format(self.project_path, md_name_3), 'w',
                              encoding='utf-8') as files:
    ...
    def operat_md_media(self, md_content):
        ...
        if len(media_list) > 0:
            for media in media_list:
                media_filename = media.split("(")[-1].split(")")[0]
                if media_filename.startswith("/"):
                    sub_folder = "/" + media_filename.split("/")[3]
                    ...
                    try:
                        shutil.copy(
                            settings.BASE_DIR + media_filename, self.media_path + sub_folder
                        )
...
class ReportEPUB():
    ...
    def write_html(self, d, html_str):
        ...
        src_tag = html_soup.find_all(lambda tag: tag.has_attr("src"))
        mindmap_tag = html_soup.select('svg.mindmap')
        tex_tag = html_soup.select('.editormd-tex')
        flowchart_tag = html_soup.select('.flowchart')
        seque_tag = html_soup.select('.sequence-diagram')
        code_tag = html_soup.find_all(name="code")

        style_link = html_soup.new_tag(name='link', href="../Styles/style.css",
                                       rel="stylesheet", type="text/css")
        katex_link = html_soup.new_tag(name='link', href='../Styles/katex.css',
                                       rel="stylesheet", type="text/css")
        editormd_link = html_soup.new_tag(name='link', href='../Styles/editormd.css',
                                          rel="stylesheet", type="text/css")
        ...
        for src in src_tag:
            if src['src'].startswith("/"):
                src_path = src['src']
                src_filename = src['src'].split("/")[-1]
                src['src'] = '../Images/' + src_filename
                try:
                    shutil.copyfile(
        ...
        for seque in seque_tag:
            ...
            seque.string = ''
            seque_img_tag = html_soup.new_tag(name='img', src='../Images/' +
                                                              seque_img_filename)
            seque.insert(0, seque_img_tag)
            os.remove(temp_seque_html)
    ...
    def generate_html(self):
        data = Doc.objects.filter(top_doc=self.project.id, parent_doc=0, status=1) \
            .order_by("sort")
        self.toc_list = [
            {
                'id': 0,
        ...
        '''
        spine = '<itemref idref="book_cover" linear="no"/><itemref ' \
                'idref="book_title"/><itemref idref="book_desc"/><itemref ' \
                'idref="toc_summary"/>'
        ...
        for d in data:
            ...
            toc_summary_str += '''<li><a href="./{}">{}</a>'''.format(toc['link'],
                                                                      toc['title'])
            manifest += '<item id="{}" href="Text/{}.xhtml" ' \
                        'media-type="application/xhtml+xml"/>'.format(d.id, d.id)
            ...
            for d2 in data_2:
                ...
                toc_summary_str += '''<li><a href="./{}">{}</a>'''.format(toc['link'],
                                                                          toc['title'])
                manifest += '<item id="{}" href="Text/{}.xhtml" ' \
                            'media-type="application/xhtml+xml"/>'.format(d2.id,
                                                                          d2.id)
                ...
                for d3 in data_3:
                    ...
                    toc_nav = '''<navPoint id="np_{nav_num}" playOrder="{nav_num}">
                        ...
                        '''.format(nav_num=nav_num, title=d3.name,
                                   file=toc['link'])
                    nav_str += toc_nav

                    toc_summary_str += '''<li><a href="./{}">{}</a></li>''' \
                        .format(toc['link'], toc['title'])
                    manifest += '<item id="{}" href="Text/{}.xhtml" ' \
                                'media-type="application/xhtml+xml"/>'.format(d3.id,
                                                                              d3.id)
    ...
    def generate_epub(self):
        try:
            zipfile_name = settings.MEDIA_ROOT + '/report_epub/{}' \
                .format(self.project.name) + '_' + str(
                int(time.time()))
    ...
    def work(self):
        ...
        self.generate_metatype()
        self.generate_toc_ncx()
        self.generate_toc_html()
        self.generate_cover()
        self.generate_title_html()
        self.generate_opf()
...
class ReportDocx():
    ...
    def work(self):
        ...
        src_tag = soup.find_all(lambda tag: tag.has_attr("src"))
        print(src_tag)
